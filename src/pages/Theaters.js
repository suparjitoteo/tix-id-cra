...
export default function Theaters({ city }) {
  ...
  return (
    <div className='flex mx-2 md:m-0 flex-wrap justify-between'>
      { theaters.map(({ id, name, address, contact }) => (
        <Link
          to={`/theaters/${id}`}
          key={id} 
          className='group px-2 mb-2 flex w-full md:w-1/3 transform hover:scale-105 transition ease-in-out duration-300 cursor-pointer'
        >
